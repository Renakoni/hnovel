...
域名失效、请求被拦截、登录页面或环境缺少 WebView 误算成解释器缺陷。本轮以已确认的
通用语义修复和主要可用书源回归为完成条件，不要求 6,636 条原始定义全部联网成功。
...
PR #189 首轮 CI 的 JVM 检查通过；API 24 和 API 35 各有 39/40 项通过，失败均为
`textParserAndSelectorInputStayLocalAcrossBinderCalls`。旧测试将解析出的文字保存为共享库
全局变量 `title`，与本轮补齐的章节绑定同名。参考 `AnalyzeRule.evalJS` 明确将 `title`
绑定为 `chapter?.title`，无章节时的 null 也优先于共享库变量；产品结果符合该语义。
测试改用 `parsedTitle`，保留原有文字、URL 和隔离边界断言；另加同输入作用域对照，
验证章节绑定覆盖共享库同名变量、库函数仍使用自身作用域、连续调用不会残留上一章。
...
- 首轮相关 JVM 测试 426 项通过；CI 测试修正后重跑 Rhino 135 项全部通过，Android APK
