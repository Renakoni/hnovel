...
#218 将普通原生登录／重新登录改为先重置旧账号环境，再打开登录页。已有待验证 URL 仍沿用其账号；关闭原生窗口保留当前会话。主动重新登录之后遇到连接失败，显示新账号的“未登录”；显式登录收到 401 则显示“需要重新登录”。这来自用户发起的账号重置，不是根据普通网络错误判断原会话失效。数据保留范围见[网页数据保留](SOURCE_WEB_STORAGE.md)。
