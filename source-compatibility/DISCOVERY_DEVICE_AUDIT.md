...
基线为 main `ad97a20d`（PR #198），交付前同步到 `7acb54fb`（包含 #195 分类标签布局）。使用六份原始 JSON，先对照参考源码，再通过 MuMu 中安装的 debug APK 测试。静态数量不代表联网通过率。
...
后续实现与验收集中跟踪在 [issue #204](https://github.com/Renakoni/hnovel/issues/204)。
...
对于 `exploreUrl` 缺失、空字符串或纯空白：**不显示发现/分类 tab，保留搜索能力**。当前 `RuleSource.canDiscover` → 注册 `SourceCapability` → `discoverySources()` 已落实此策略。本轮新增真实 importer/注册链路的回归，覆盖 `enabledExplore=true` 但没有入口的三种情况，并实际调用其搜索确认能力仍在。

这与“目录解析失败”“列表请求失败”“动态目录本次返回空”不同，后三者不能因为一次失败就隐藏整个源。静态 `[]` / 纯占位目录与动态空结果尚未做统一的可用性归类，列入下一阶段：先区分无导航、只有交互控件、暂时无内容，再决定是否隐藏，避免登录前为空的目录永久消失。

## 实测结果

MuMu Android 12L（API 32）先测同一组 12 个源的原始定义，再安装修复 APK 重放。两份同名刺猬猫按 www/wap 版本区分。

| 样本 | 修复后目录入口数 | 发现预览书籍数 | 前两个有效入口的书籍数 |
| --- | ---: | ---: | --- |
| 52书库 | 6 | 0（保留预览错误） | 0 / 0 |
| 晋江 | 1 | 6 | 6 |
| QQ阅读 | 353 | 6 | 20 / 20 |
...
12/12 目录可解析，10/12 首页取得预览，11/12 至少一个所测入口返回书籍。不是六份文件全部书源的通过率，也不代表任意入口、章节都成功。

参考阅读同入口验证：豆瓣、番茄、刺猬猫榜单成功；52书库两个入口也未返回书籍；阅友首榜在参考端抛 JSON 路径异常，产品返回空，后续女频入口可读取。原始定义保持不变，没有为凑通过率修改坏网址或换规则。52书库最初搜索权限缺少已知重定向域名，补齐诊断输入的显式授权后搜索返回 10 本；该准备错误单独记录，不算解释器修复。

晋江搜索由 `ValidationException` 恢复到 50 本。刺猬猫 www 曾有一次搜索返回空，当时响应只有约 6 KB，之前成功响应约 81 KB；在另一台 MuMu 再测恢复到 10 本，榜单仍为 15 / 10 本。这一响应差异不能当作选择器已报错。额外点击的“未来幻想”分类在产品和参考端都为空，也不计为成功书单。52书库的另一台 MuMu 复测仍可搜索，但三个发现请求均网络超时，保留错误，不宣称该源发现已修复。

UI 在另一台 MuMu 中验证：刺猬猫榜单预览可见，“查看更多”进入有书籍卡片的结果页；分类页面保留 #195 的紧凑标签并可导航，空分类显示空结果；原生 Wenku8 的 29 个分类标签可见。测试前备份数据，临时聚焦 12 个样本，结束后恢复原有发现显示设置。

本地验证：完整 JVM 897 项通过；同步 #195 后再跑分类、发现与 adapter 34 项通过；新增无发现能力测试后 adapter 20 项通过。API 35 的现有设备测试集合共 55 项通过，新增 Jsoup 排除规则在 MuMu isolated worker 单独执行也通过。
