...
| 验证恢复 | PR #188、前台 UI、后台提示、导航/账号取消 | 可控本地挑战完整恢复；真实 hlib 一次用户操作继续读取；多来源不串回 | 8 项协调器单测、确切请求/过期来源规则测试、MuMu 界面重建→验证→Cookie→原搜索均通过；此前实站等待已超时，完整自动恢复尚未验收 |
...
实站通过生产注册表、真实导入源与协调器到达“验证并继续”，点击后打开该账号的 hlib CF 页面；本次人工等待在 350 秒后超时结束，未通过搜索恢复验收。此前 #187 的完整实站读取仍只是基线。
