...
(async()=>{
 ...
 args.push('indi.renakoni.nextvol.debug.test/androidx.test.runner.AndroidJUnitRunner');
 ...
 for(let attempt=0;attempt<35;attempt++){
  await delay(500);
  try{
   const pid=command('shell','pidof','indi.renakoni.nextvol.debug:source_browser_native').trim();
