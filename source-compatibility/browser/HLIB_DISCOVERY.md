...
- 标签分类不提供页码输入、载入按钮或文章排序，只显示标签与上一页／下一页。分类操作固定在来源 tab 下方，以可横向滚动的紧凑按钮展示；滚动标签后仍可直接翻页，保留长按和加载期间禁用行为。
...
MuMu 本次真实发现页触发 CF 后自动打开网站窗口；用户通过后自动返回首页，四个入口均有预览，文章从最新切到收藏数可加载，标签结果可打开。生产规则实测日榜29、周榜30、月榜30、文章27项；标签第一页100项，第二页与对应列表通过。完整读取另行复核为搜索30项、目录63章、首章608段/34346字符；配套 APK 的自动恢复 fixture 同次通过（2 tests，55.919秒）。没有记录站点正文、密码或 Cookie 值。
