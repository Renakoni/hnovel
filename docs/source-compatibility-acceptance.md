...
首次执行缺少固定 Java 互操作绑定时，报告 `UnsupportedDependency`、具体依赖与字段（共享库为 `jsLib`），并给出当前 profile 和规则改写方向。复杂番茄原文件的初始化、后续控制实验与最新网络对照见 [分阶段兼容评估](fq-complex-source-compatibility.md)；可导入不等于已可搜索或阅读。
