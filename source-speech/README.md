...
Real-service coverage is recorded in [ACCEPTANCE.md](ACCEPTANCE.md) and the per-configuration [acceptance.json](acceptance.json), tracked by issue #252. On 2026-09-19, 98 configurations across seven service families completed application synthesis, full decoding, non-silent PCM validation and production playback, including all 26 Microsoft configurations. The reference sample size is not a delivery quota. One selection script remains unsupported, three tested endpoints/configurations were excluded, and one requires user credentials. The credential-bearing source asset and live probe are not shipped.
